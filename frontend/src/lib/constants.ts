...
export const FIELD_TOOLTIPS: Record<keyof SMEFinancialData, string> = {
  ...
  roa_b:
    "Return on Assets (B) — similar to ROA(A) but adds back depreciation before the tax calculation, giving a cash-flow-oriented profitability view. Higher is healthier. Distressed companies typically show values well below the dataset median.",
